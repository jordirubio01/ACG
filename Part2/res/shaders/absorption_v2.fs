...
// --- new uniforms for heterogeneous model ---
uniform bool u_absorption_type;     // 0 -> homogeneous, 1 -> heterogeneous
...
void main()
{
    ...
    if (u_absorption_type == 1) {
        ...
        float transmittance = 1.0; // inicialment tot passa (cap absorció)
        float t = tnear + step_size * 0.5  ;             // start from the bottom 

        while (t < tfar && transmittance > 0.001) {
            vec3 sample_pos = origin_local + direction_local * t;

            // mostra la densitat amb soroll
            float density = max(0.0, snoise(sample_pos * u_noise_freq));
            float mu = density * u_density_scale;

            t += step_size;
            if (transmittance < 0.001) break; // early exit si quasi opac
        }

        transmittance = exp(-mu)
